...
<body>
	...
	<s:if test="#session.status=='1'.toString()">
		<div id="mainContainer">
			<table class="default" width="80%" height="50px">
				...
				<s:iterator value="#session.my_room_list" var="room">
					<tr class="list">
						...
						<s:if test='%{#room.status=="故障"}'>
							<td>您已经报修</td>
						</s:if>
						<s:else>
							<td>您正在使用</td>
						</s:else>
						<td><a href="<%=path%>/members/Member_report.action?RId=<s:property value="#room.RId"/>">我要报修</a></td>
						<td><a href="<%=path%>/members/Member_checkout.action?RId=<s:property value="#room.RId"/>">我要退房</a></td>
					</tr>
				</s:iterator>
			</table>
		</div>
	...
	<s:else>
	<p align="center">您是系统管理员，无法订房间</p>
	</s:else>
</body>
</html>